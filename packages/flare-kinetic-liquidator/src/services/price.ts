...
export async function priceMantissa(cToken: string, provider: Provider): Promise<bigint> {
  const oracle = await oracleForCToken(cToken, provider);
  const p: bigint = await oracle.getUnderlyingPrice(cToken);
...
export async function borrowUsd18WithOracle(user: string, cToken: string, provider: Provider, oracleAddr: string): Promise<bigint> {
  const ct = new Contract(cToken, CTOKEN_MIN_ABI, provider);
